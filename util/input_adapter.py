...
class EEGDataAdapter:
  ...
  def __init__(self, device_type='Unicorn', channel_indices = [1,2,3,4,5,6,7,8], mode='offline', number_of_samples=125, event_dict = None):
  ...
  def online_streamer(self):
    ...
    if self.device_type == 'Unicorn':
      ...
      try:
        samples_buffer_numpy, timestamps_buffer_numpy = np.array(self.samples_buffer)[: ,self.channel_indices].T, np.array(self.timestamps_buffer) 
      except IndexError:
  ...
  def load_recorded_eeg_data_file(self, file_name = "../data/B0101T", file_type='gdf', recording_paradigm='', tmin=0, tmax=2): 
    ...
    if file_type=='edf':
      ...
      if self.device_type=='Unicorn':
          ...
          raw_data.rename_channels({'Channel 1':  'EEG Fz',    'Channel 2':  'EEG C3',  'Channel 3':  'EEG Cz',        'Channel 4':  'EEG C4',
                      'Channel 5':  'EEG Pz',    'Channel 6':  'EEG PO7', 'Channel 7':  'EEG Oz',        'Channel 8':  'EEG PO8',
                      'Channel 9':  'Accelerometer X',   'Channel 10': 'Accelerometer Y', 'Channel 11': 'Accelerometer Z','Channel 12': 'Gyroscope X',
                      'Channel 13': 'Gyroscope Y',    'Channel 14': 'Gyroscope Z',  'Channel 15': 'Battery LeveL', 'Channel 16': 'Counter',
    ...
    epochs = Epochs(raw_data, self.events, self.event_dict, tmin=tmin, tmax=tmax, proj=False, baseline=None, preload=True, picks=picks)
  ...
  def select_events(self, selected_events=['break', 'imagery_handR', 'imagery_foot', 'imagery_handL'], raw_type='xdf'):
